const signupUser = async ({
  ...
  try {
    ...
    const response = await fetch("http://192.168.10.134:3000/users/signup", {
      method: "POST",
      headers: {
...
const loginUser = async ({ email, password }) => {
  try {
    ...
    // Make the fetch request with the FormData object as request body
    const response = await fetch("http://192.168.10.134:3000/users/signin", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
const updateUserPictures = async (usertoken, arrayOfPictures) => {
  try {
    ...
    const response = await fetch(
      "http://192.168.10.134:3000/users/uploadPictures",
      {
        method: "POST",
        body: formData,
      }
